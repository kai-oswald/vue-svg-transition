<template>
<svg class="js-transition-icon" :width="size.width + 'px'" :height="size.height + 'px'" :viewBox="'0 0 ' + size.width + ' ' + size.height" @click="perfomTransition">
...
export default {
  ...
  data() {
    return {
     states: [],
     time: { start: null, total : this.duration },
     status: { interacted: false, animating: false}
    };
  },
  methods: {
    perfomTransition() {
